...
public class AdaptiveAttribute {
  public static void constructor(Rand rn, int[] crm, int base, int attribute,
                                 InstanceWrapper ins) {
    if (ParallelGlobals.getGlobals_ADI().types[attribute] == Attribute.REAL) {
      int disc = rn.getInteger(0
                                 ,
      ...
      if (maxAllowedIntervals > 2) {
        numIntervals = rn.getInteger(2, maxAllowedIntervals);
      }
      else {
      ...
      for (i = 0, pos = base + 2, max = numIntervals - 1; i < max; i++,
        ...
        if (!done && intervalOfSample < sum) {
          ...
          if (rn.getReal() < Parameters.probOne) {
            crm[pos + 1] = 1;
          }
          else {
            crm[pos + 1] = 0;
      ...
      crm[base] = crm[base + 1] = ParallelGlobals.getGlobals_ADI().size[attribute] - 2;
...
//    }
  ...
  public static int doSplit(Rand rn, int[] crm, int base, int attribute, int interval) {
    if (ParallelGlobals.getGlobals_ADI().types[attribute] != Attribute.REAL) {
      return 0;
    }
    ...
    int cutPoint = rn.getInteger(0, size - 2);
  ...
  public static int doMerge(Rand rn, int[] crm, int base, int attribute, int interval) {
    if (ParallelGlobals.getGlobals_ADI().types[attribute] != Attribute.REAL) {
      return 0;
    }
    if (crm[base] == 1) {
  ...
  public static void doReinitialize(Rand rn, int[] crm, int base, int attribute) {
    if (ParallelGlobals.getGlobals_ADI().types[attribute] != Attribute.REAL) {
      return;
    }
    constructor(rn, crm, base, attribute, null);
  }
}
